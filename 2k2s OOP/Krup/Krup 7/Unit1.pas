...
begin
  ...
  begin
    ...
    ButtonShow.Caption := 'Показать окно';
    Exc.Quit;
    closed:=true;
  end;
...
if closed then
   begin
     ShowMessage('Excel закрыт (Проверьте все ли данные введены и нажмите кнопку "Решить")');
     exit;
   end;
  ...
  begin
    ButtonShow.Caption:='Скрыть окно';
    Exc.Visible:=True;
  end;
end;
...
if Edita33.Text='' then
  ...
  Except
    ShowMessage('В следующий раз лучше закрывайте Exel с помощью копки "Закрыть" =)');
    Exc.Visible:=False;
    Button_close.Enabled := False;
    ButtonShow.Caption := 'Показать окно';
    Exc.Quit;
