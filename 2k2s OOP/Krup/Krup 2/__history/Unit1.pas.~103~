...
begin
  ...
  begin
    ShowMessage('Ошибка ввода! Проверьте заполнение обоих полей!');
    exit
  end;

  if (strtofloat(edit2.Text) <= 0) or (strtofloat(edit2.Text) >= 1) then
  begin
    ShowMessage('Допустимое значение для точности: (0;1)');
    exit;
  end;
  ...
  if (edit3.Enabled = true) and (edit3.text <> '') then
    ...
    showmessage('Введите значение A');
  ...
  Label5.Caption := 'Сумма ряда:' + floattostr(Sum);
  Label5.Visible := True;
